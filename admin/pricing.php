...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_group'])) {
  ...
  if (empty($groupName)) {
    ...
    header('Location: pricing.php?group_added=1&tab=group_' . $pdo->lastInsertId());
    exit;
  }
}
...
if (isset($_GET['delete_group'])) {
  ...
  header('Location: pricing.php?group_deleted=1&tab=global');
  exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add'])) {
  ...
  if (!$day_type || !$start_time || !$end_time) {
    ...
    try {
      ...
      header("Location: pricing.php?added=1&tab=$back_tab");
      exit;
...
if (isset($_GET['delete'])) {
  ...
  header("Location: pricing.php?deleted=1&tab=$back_tab");
  exit;
}

if (isset($_GET['added']))
  $success = 'เพิ่มกฎราคาสำเร็จ';
if (isset($_GET['deleted']))
  $success = 'ลบกฎราคาสำเร็จ';
if (isset($_GET['group_added']))
  $success = 'สร้างกลุ่มราคาสำเร็จ';
if (isset($_GET['group_deleted']))
  $success = 'ลบกลุ่มราคาสำเร็จ';
...
<body style="background:#FAFAFA;" class="min-h-screen">
  ...
  <div class="max-w-7xl mx-auto px-4 py-6">
    ...
    <div id="newGroupPanel"
      class="<?= (isset($_GET['group_added']) || $error) ? '' : 'hidden' ?> mb-5 bg-white rounded-xl border border-gray-200 p-5">
            ...
            <div class="grid grid-cols-1 xl:grid-cols-5 gap-6">
              ...
              <div class="xl:col-span-3 space-y-4">
                ...
                <div class="bg-gray-50 rounded-xl border border-gray-200 p-4">
                  <p class="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">ภาพรวมช่วงเวลา (06:00–23:00)
                  </p>
                          ...
                          <div title="<?= $slot['start'] ?>–<?= $slot['end'] ?> = ฿<?= number_format($slot['price'], 0) ?>/ชม."
                            ...
                            <?php if ($slot['width'] > 8): ?>
                              <span class="text-white text-xs font-bold drop-shadow"
                                style="font-size:10px;">฿<?= number_format($slot['price'], 0) ?></span>
                            <?php endif; ?>
                            <!-- tooltip -->
                            <div
                              class="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 hidden group-hover:block z-10 bg-gray-900 text-white text-xs rounded px-2 py-1 whitespace-nowrap pointer-events-none">
                        ...
                        foreach ($sorted as $r):
                          ...
                          <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-4 py-2.5">
                              <span
                                class="text-xs px-2 py-0.5 rounded-full font-medium <?= $isWe ? 'bg-orange-100 text-orange-700' : 'bg-blue-100 text-blue-700' ?>">
                                <?= $isWe ? 'เสาร์–อาทิตย์' : 'จันทร์–ศุกร์' ?>
                              </span>
                            </td>
                            <td class="px-4 py-2.5 text-center">
                              <span class="font-mono text-xs text-gray-700"><?= substr($r['start_time'], 0, 5) ?> –
                                <?= substr($r['end_time'], 0, 5) ?></span>
                            </td>
                  ...
                  <form method="post" class="space-y-3" id="form-<?= $tabId ?>">
                    ...
                    <div>
                      ...
                      <div class="grid grid-cols-2 gap-2">
                        <label
                          class="day-btn flex items-center justify-center gap-2 px-3 py-2.5 border-2 border-blue-200 bg-blue-50 text-blue-700 rounded-lg cursor-pointer text-xs font-medium hover:border-blue-400 transition-colors">
                        ...
                        <label
                          class="day-btn flex items-center justify-center gap-2 px-3 py-2.5 border-2 border-gray-200 text-gray-500 rounded-lg cursor-pointer text-xs font-medium hover:border-orange-300 transition-colors">
    ...
    <div id="pickerModal" class="hidden fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4"
      onclick="closePicker(event)">
      ...
      <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-xs" onclick="event.stopPropagation()">
        ...
        <div class="grid grid-cols-4 gap-1 p-3 max-h-64 overflow-y-auto" id="pickerGrid">
          <?php foreach ($timeOptions as $t): ?>
            <button type="button" data-time="<?= $t ?>" onclick="selectTime('<?= $t ?>')"
  ...
  <style>
    ...
    .day-btn input[type=radio]:checked+svg {
      display: block;
    }
  </style>
