...
if ($pkg_id <= 0) {
    echo json_encode(['success' => false, 'message' => 'ไม่ระบุ pkg_id'], JSON_UNESCAPED_UNICODE);
    exit;
}

$pkgStmt = $pdo->prepare('SELECT id, payment_slip_path FROM member_badminton_packages WHERE id = ?');
$pkgStmt->execute([$pkg_id]);
$pkg = $pkgStmt->fetch();

if (!$pkg) {
    echo json_encode(['success' => false, 'message' => 'ไม่พบแพ็กเกจนี้'], JSON_UNESCAPED_UNICODE);
    exit;
}
...
$filename  = 'badminton_slip_' . $pkg_id . '_' . uniqid() . '.' . $ext;
...
// ลบสลิปเก่า
if (!empty($pkg['payment_slip_path'])) {
    $oldPath = __DIR__ . '/../' . $pkg['payment_slip_path'];
    if (file_exists($oldPath)) @unlink($oldPath);
}

$relPath = 'uploads/slips/' . $filename;
$pdo->prepare('UPDATE member_badminton_packages SET payment_slip_path = ? WHERE id = ?')
    ->execute([$relPath, $pkg_id]);
